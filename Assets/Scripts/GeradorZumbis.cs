...
public class GeradorZumbis : MonoBehaviour
{
    ...
    public LayerMask LayerZumbiECenario;
    private float diatanciaDeGeracao = 3;
    ...
    void Update()
    {
            ...
            quantidadeDeZumbisVivos < quantidadeMaximaDeZumbisVivos)
        {
            ...
            {
                StartCoroutine(GerarNovoZumbi()); //anti-bug
                contadorTempo = 0;
            }
        }
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, diatanciaDeGeracao);
    }

    IEnumerator GerarNovoZumbi() //anti-bug
    {
        Vector3 posicaoDeCriacao = AleatorizarPosicao();
        Collider[] colisores = Physics.OverlapSphere(posicaoDeCriacao, 1, LayerZumbiECenario);

        while (colisores.Length > 0)
        {
            posicaoDeCriacao = AleatorizarPosicao();
            colisores = Physics.OverlapSphere(posicaoDeCriacao, 1, LayerZumbiECenario);
            yield return null; //anti-bug
        }
    ...
    {
        Vector3 posicao = Random.insideUnitSphere * diatanciaDeGeracao;
